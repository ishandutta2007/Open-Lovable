...
testSkipIfWindows("local-agent - list_files", async ({ po }) => {
  ...
  const listFiles1 = po.page.getByTestId("openlovable-list-files").first();
  await listFiles1.click();
  await expect(listFiles1).toMatchAriaSnapshot();

  const listFiles2 = po.page.getByTestId("openlovable-list-files").nth(1);
...
testSkipIfWindows("local-agent - list_files include_hidden", async ({ po }) => {
  await po.setUpOpenLovablePro({ localAgent: true });
  await po.importApp("minimal-with-openlovable");
  await po.chatActions.selectLocalAgentMode();

  await po.sendPrompt("tc=local-agent/list-files-include-hidden");
  const listFiles = po.page.getByTestId("openlovable-list-files").first();
