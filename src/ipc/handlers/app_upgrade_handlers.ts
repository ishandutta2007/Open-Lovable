...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Open-Lovable component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://openlovable.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl: "https://openlovable.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes("@openlovable-sh/react-vite-component-tagger");
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (
    !content.includes(
      "import openlovableComponentTagger from '@openlovable-sh/react-vite-component-tagger';",
    )
  ) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("openlovableComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [openlovableComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @openlovable-sh/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @openlovable-sh/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[openlovable] add Open-Lovable component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[openlovable] add Capacitor for mobile app support",
    });
