...
const listFilesSchema = z.object({
  ...
  include_hidden: z
    ...
    .optional()
    .describe(
      "Whether to include .openlovable files which are git-ignored (default: false)",
    ),
});
...
export const listFilesTool: ToolDefinition<ListFilesArgs> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<openlovable-list-files${getXmlAttributes(args)}></openlovable-list-files>`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    // If include_hidden is true, also include .openlovable files
    if (args.include_hidden) {
      const normalizedAppPath = ctx.appPath.replace(/\\/g, "/");
      // Always search .openlovable at the app root, regardless of directory filter
      const openlovableGlobPattern = `${normalizedAppPath}/.openlovable${globSuffix}`;

      const openlovableFiles = await glob(openlovableGlobPattern, {
        nodir: true,
        absolute: true,
        ignore: [
          "**/node_modules/**",
      ...
      const openlovableRelativePaths = openlovableFiles.map((file) =>
        path.relative(ctx.appPath, file).split(path.sep).join("/"),
      );

      // Deduplicate and sort
      allFilePaths = [
        ...new Set([...allFilePaths, ...openlovableRelativePaths]),
      ].sort();
    }
    ...
    ctx.onXmlComplete(
      `<openlovable-list-files${getXmlAttributes(args)}>${escapeXmlContent(abbreviatedList + countInfo)}</openlovable-list-files>`,
    );
