...
import { type UserSettings, hasOpenLovableProKey } from "@/lib/schemas";
...
export function useSettings() {
  ...
  useEffect(() => {
    if (settingsQuery.data) {
      ...
      const isPro = hasOpenLovableProKey(settingsQuery.data);
  ...
  const updateSettingsMutation = useMutation({
    ...
    onSuccess: (updatedSettings) => {
      ...
      posthog.people.set({ isPro: hasOpenLovableProKey(updatedSettings) });
