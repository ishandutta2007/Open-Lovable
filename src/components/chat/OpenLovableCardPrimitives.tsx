...
/**
 * Accent color configuration for OpenLovableCard components.
 * Maps to Tailwind color classes for border, background, and text.
 */
export type OpenLovableAccentColor =
  | "blue"
  | "purple"
  | "violet"
  | "red"
...
const ACCENT_BORDER: Record<OpenLovableAccentColor, string> = {
...
const ACCENT_ICON_BG: Record<OpenLovableAccentColor, string> = {
...
const ACCENT_BADGE: Record<OpenLovableAccentColor, string> = {
...
interface OpenLovableCardProps {
  children: React.ReactNode;
  state?: CustomTagState;
  accentColor?: OpenLovableAccentColor;
  showAccent?: boolean;
 ...
 */
export function OpenLovableCard({
  children,
  state,
  accentColor = "blue",
  showAccent,
...
interface OpenLovableCardHeaderProps {
...
/**
 * Header row for OpenLovableCard. Contains a tinted icon circle and flexible content area.
 */
export function OpenLovableCardHeader({
  icon,
  accentColor = "blue",
  children,
}: OpenLovableCardHeaderProps) {
  return (
...
interface OpenLovableBadgeProps {
  children: React.ReactNode;
  color?: OpenLovableAccentColor;
}

/**
 * Small pill badge for labeling card types (e.g. "GREP", "Turbo Edit", "SQL").
 */
export function OpenLovableBadge({ children, color = "blue" }: OpenLovableBadgeProps) {
  return (
    <span
...
interface OpenLovableStateIndicatorProps {
  state: CustomTagState;
  pendingLabel?: string;
  abortedLabel?: string;
 ...
 */
export function OpenLovableStateIndicator({
  state,
  pendingLabel,
  abortedLabel,
  finishedLabel,
}: OpenLovableStateIndicatorProps) {
  if (state === "pending") {
    return (
...
interface OpenLovableCardContentProps {
  isExpanded: boolean;
  children: React.ReactNode;
  className?: string;
}
 ...
 */
export function OpenLovableCardContent({
  isExpanded,
  children,
  className = "",
}: OpenLovableCardContentProps) {
...
interface OpenLovableFilePathProps {
  path: string;
}
 ...
 */
export function OpenLovableFilePath({ path }: OpenLovableFilePathProps) {
  if (!path) return null;
  return (
...
interface OpenLovableDescriptionProps {
  children: React.ReactNode;
}
 ...
 */
export function OpenLovableDescription({ children }: OpenLovableDescriptionProps) {
  return (
