...
interface ErrorBannerProps {
  error: { message: string; source: "preview-app" | "openlovable-app" } | undefined;
  onDismiss: () => void;
  onAIFix: () => void;
}
...
const ErrorBanner = ({ error, onDismiss, onAIFix }: ErrorBannerProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Add a little chip that says "Internal error" if source is "openlovable-app" */}
      {error.source === "openlovable-app" && (
      ...
      <div
        className={cn(
          "px-6 py-1 text-sm",
          error.source === "openlovable-app" && "pt-6",
        )}
      >
        <div
        ...
        <div className="relative p-2 bg-red-100 dark:bg-red-900 rounded-sm flex gap-1 items-center">
          ...
          <span className="text-sm text-red-700 dark:text-red-200">
            ...
            {isDockerError
              ...
              : error.source === "openlovable-app"
                ? "Try restarting the Open-Lovable app or restarting your computer to see if that fixes the error."
...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
    ...
    try {
      ...
      if (result.hasStaticText && iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          {
            type: "enable-openlovable-text-editing",
            data: {
  ...
  useEffect(() => {
    if (iframeRef.current?.contentWindow && isComponentSelectorInitialized) {
      iframeRef.current.contentWindow.postMessage(
        { type: "openlovable-pro-mode", enabled: isProMode },
        "*",
      );
    }
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "openlovable-component-selector-initialized") {
      ...
      if (event.data?.type === "openlovable-text-updated") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "openlovable-text-finalized") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "openlovable-component-selected") {
      ...
      if (event.data?.type === "openlovable-component-deselected") {
        ...
        if (componentId) {
          ...
          if (iframeRef.current?.contentWindow) {
            iframeRef.current.contentWindow.postMessage(
              {
                type: "disable-openlovable-text-editing",
                data: { componentId },
              },
              "*",
            );
      ...
      if (event.data?.type === "openlovable-component-coordinates-updated") {
      ...
      if (event.data?.type === "openlovable-screenshot-response") {
  ...
  const handleActivateComponentSelector = () => {
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-openlovable-component-selector"
            : "deactivate-openlovable-component-selector",
        },
        "*",
      );
    }
  ...
  const handleAnnotatorClick = () => {
    ...
    if (iframeRef.current?.contentWindow) {
      iframeRef.current.contentWindow.postMessage(
        {
          type: "openlovable-take-screenshot",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (!data || data.type !== "openlovable-component-selected") {
    return null;
  }
