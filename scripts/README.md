...
1. `openlovable-{version}-1.x86_64.rpm` (Linux RPM)
2. `openlovable-{version}-full.nupkg` (Windows NuGet package)
3. `openlovable-{version}.Setup.exe` (Windows installer)
4. `openlovable-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `openlovable-darwin-x64-{version}.zip` (macOS Intel)
6. `openlovable_{version}_amd64.deb` (Linux DEB)
